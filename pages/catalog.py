"""
pages/catalog.py — the catalog (read) page. Depends only on
data.load_catalog() + config. Never reference a concrete data source, path,
table name, or credential here — that all lives in config.py.
"""
...
def _pill_row(marker_key: str, values: list, selected: str, on_pick) -> None:
    with st.container():
        ...
        for col, value in zip(cols, values):
            with col:
                if theme.toggle_button(
                    value, key=f"{marker_key}_{value}", active=(selected == value),
...
_pill_row("product", [ALL_PRODUCTS] + list(config.STRUCTURE_DATABASES), selected_product, _pick_product)
...
if selected_product != ALL_PRODUCTS:
    ...
    if schema_names:
        ...
        _pill_row("schema", [ALL_SCHEMAS] + schema_names, selected_schema, _pick_schema)
...
if selected_product != ALL_PRODUCTS and selected_schema != ALL_SCHEMAS:
    ...
    if table_names:
        ...
        _pill_row("table", [ALL_TABLES] + table_names, selected_table, _pick_table)
