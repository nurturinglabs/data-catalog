"""
pages/workspace.py — the Documentation workspace: the authoring/write side
...
# ─────────────────────────────────────────────────────────────────────────────
# Identity — SiS resolves CURRENT_USER() automatically; locally, a picker.
# ─────────────────────────────────────────────────────────────────────────────
...
if not workflow.is_sis():
    ...
    picked = st.selectbox(
        "Acting as", assignees, index=default_index, key="workspace_user_picker",
    )
...
role = st.radio(
    "Role", ["Coordinator", "Assignee"], key="workspace_role",
...
def render_coordinator(df: pd.DataFrame) -> None:
    ...
    with filter_cols[0]:
        status_filter = st.multiselect("Status", workflow.STATUSES, key="coord_status_filter")
    with filter_cols[1]:
    ...
    with action_cols[0]:
        ...
        if st.button("Assign selected", key="coord_bulk_assign_btn", use_container_width=True):
    ...
    with action_cols[1]:
        if st.button("Approve selected", key="coord_bulk_approve_btn", use_container_width=True):
            ...
            with c1:
                if st.button("Save draft", key=f"save_draft_{col}", use_container_width=True):
                    try:
                        ...
                        st.error(f"{exc} Reload to see the latest version before retrying.")
            with c2:
                if st.button("Submit", key=f"submit_{col}", use_container_width=True):
